...
import { 
  ...
  Image as ImageIcon // CORRECTION : On renomme l'icône pour ne pas la confondre avec le composant Image
} from 'lucide-react'
...
export default function MarketplaceHome() {
  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-24 font-sans relative">
      ...
      {/* --- FILTRES ÎLES --- */}
      {/* CORRECTION LINTER : top-30 au lieu de top-[120px] */}
      <div className="bg-white border-b border-gray-100 py-3 sticky top-30 z-40 shadow-sm">
                ...
                displayProducts.map((product) => {
                    ...
                    return (
                        <Link key={product.id} href={`/annonce/${product.id}`} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col hover:shadow-md transition-shadow relative group">
                            <button onClick={(e) => toggleFavorite(e, product.id)} className="absolute top-2 right-2 z-20 bg-white/90 backdrop-blur-sm p-1.5 rounded-full shadow-sm hover:bg-white active:scale-90 transition">
                                <Heart size={16} className={isFav ? "fill-red-500 text-red-500" : "text-gray-400"} />
                            </button>
                            
                            {/* CORRECTION LINTER : aspect-4/3 au lieu de aspect-[4/3] */}
                            <div className="relative aspect-4/3 bg-gray-100">
                                {imageUrl ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center h-full text-gray-300">
                                        {/* CORRECTION : Utilisation de ImageIcon (Lucide) et non Image (Next.js) */}
                                        <ImageIcon size={24} />
      ...
      {/* --- MODAL PLEIN ÉCRAN --- */}
      {fullScreenImage && (
        // CORRECTION LINTER : z-50 au lieu de z-[100] (z-50 est le max standard)
