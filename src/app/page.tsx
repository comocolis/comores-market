...
export default function ComptePage() {
  ...
  const handleAvatarClick = () => {
    // CORRECTION : On autorise le clic SEULEMENT si le mode édition est actif
    if (isEditingInfo) {
      ...
      <div className="bg-white p-6 pb-8 rounded-b-4xl shadow-sm relative z-10">
        ...
        <div className="flex items-center gap-4">
            ...
            <div 
                ...
                className={`w-20 h-20 bg-brand/10 rounded-full flex items-center justify-center text-brand text-2xl font-bold relative overflow-hidden border-2 border-white shadow-md group ${isEditingInfo ? 'cursor-pointer' : ''}`}
            >
                {avatarUploading ? (
                ...
                {/* CORRECTION : L'icône caméra n'apparait que SI on est en mode édition */}
                {isEditingInfo && (
                    <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                        <Camera size={20} className="text-white animate-in zoom-in" />
                    </div>
                )}
