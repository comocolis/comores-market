    ...
    <main className="min-h-screen bg-gray-50 pb-24 font-sans relative">
      ...
      <div className="max-w-md mx-auto px-4 py-4">
        <div className="flex justify-between gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {CATEGORIES.map((cat) => (
                <button key={cat.label} onClick={() => handleCategoryChange(cat.label)} className="flex flex-col items-center gap-2 min-w-17.5">
        ...
        {loading ? <div className="flex justify-center py-20"><Loader2 className="animate-spin text-brand" /></div> : (
            ...
            {displayProducts.length === 0 ? (
                <div className="col-span-2 text-center py-20 text-gray-400">
                    ...
                    <button onClick={() => {setSelectedCategory(null); setSelectedIsland('Tout'); setSearchTerm('')}} className="text-brand font-bold text-sm mt-2">Tout effacer</button>
                </div>
            ) : (
