...
import { Heart, MapPin, Loader2, ShoppingBag } from 'lucide-react'
...
export default function FavorisPage() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      // On récupère les favoris ET les détails du produit associé
      const { data } = await supabase
        .from('favorites')
        .select('product:products(*)')
        .eq('user_id', user.id)
      
      // On extrait juste les produits de la réponse
