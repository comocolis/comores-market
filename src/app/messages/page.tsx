...
function MessagesContent() {
  ...
  // --- GESTION DES PARAMÈTRES POUR LA RELANCE PROSPECT ---
  useEffect(() => {
                ...
                try {
                    ...
                    if (pRes.data && uRes.data) {
                        ...
                        const stubConv: Conversation = {
                            ...
                            productTitle: pRes.data.title,
                            productImage: img,
                            productPhone: pRes.data.whatsapp_number,
                            counterpartId: targetUserId,
                            counterpartName: uRes.data.full_name,
                            counterpartAvatar: uRes.data.avatar_url,
                            counterpartIsPro: uRes.data.is_pro,
                            lastMessage: '',
  ...
  const handleCall = () => { if (!activeConv?.productPhone) { toast.error("Aucun numéro."); return }; const clean = activeConv.productPhone.replace(/\D/g, ''); window.open(`tel:${clean}`, '_self') }
  
  const confirmDeleteConversation = () => { setShowMenu(false); setShowDeleteModal(true) }
  ...
  const handleDeleteConversation = async () => { 
      ...
      else { toast.success("Discussion supprimée"); setShowDeleteModal(false); closeConversation() } 
  }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!profile?.is_pro) {
        toast.error("Réservé aux membres PRO", { duration: 5000 })
    ...
    try {
        ...
        const { error: msgError } = await supabase.from('messages').insert({ content: publicUrl, sender_id: currentUser.id, receiver_id: activeConv?.counterpartId, product_id: activeConv?.productId })
    ...
    } catch (error) { toast.error("Erreur envoi image") } 
  ...
  const handleSend = async () => {
    ...
    const content = replyContent; setReplyContent(''); inputRef.current?.focus() 
  ...
  return (
    <div className="flex flex-col h-dvh bg-[#F7F8FA] font-sans text-gray-900">
        ...
        {previewImage && (
            <div className="fixed inset-0 z-[120] bg-black animate-in fade-in duration-300">
                <div className="absolute top-0 left-0 w-full p-4 flex justify-between items-center z-50 pointer-events-none">
                    <button onClick={() => setPreviewImage(null)} className="text-white p-3 bg-black/50 rounded-full pointer-events-auto"><X size={24} /></button>
                </div>
                <TransformWrapper initialScale={1} minScale={1} maxScale={4} centerOnInit={true} wheel={{ disabled: true }}>
                    {({ zoomIn, zoomOut, resetTransform }) => (
                        <>
                            <TransformComponent wrapperStyle={{ width: "100vw", height: "100vh" }} contentStyle={{ display: "flex", alignItems: "center", justifyContent: "center", width: "100%", height: "100%" }}>
                                <img src={previewImage} alt="Zoom" className="max-h-screen max-w-full object-contain" onLoad={() => resetTransform()} />
                            </TransformComponent>
                            <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-50 flex gap-4 pointer-events-auto">
                                <button onClick={() => zoomOut()} className="bg-white/10 backdrop-blur-md p-3 rounded-full text-white"><ZoomOut size={20} /></button>
                                <button onClick={() => zoomIn()} className="bg-white/10 backdrop-blur-md p-3 rounded-full text-white"><ZoomIn size={20} /></button>
                            </div>
                        </>
                    )}
                </TransformWrapper>
        ...
        {/* MODALE SUPPRESSION */}
        {showDeleteModal && (
            <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in" onClick={() => setShowDeleteModal(false)}>
                <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 space-y-4 animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center gap-3 text-red-600"><div className="bg-red-100 p-2 rounded-full"><AlertTriangle size={24} /></div><h3 className="font-bold text-lg">Supprimer ?</h3></div>
        ...
        {/* HEADER CHAT - AVEC LIENS PROFILS */}
        <div className="bg-brand px-4 pb-3 pt-safe shadow-md flex items-center gap-3 sticky top-0 z-40 text-white min-h-20">
            <button onClick={closeConversation} className="p-2 -ml-2 text-white/80"><ArrowLeft size={22} /></button>
            <Link href={`/profil/${activeConv?.counterpartId}`} className="flex flex-1 items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-full bg-white/20 overflow-hidden relative border border-white/30 shrink-0">
                ...
                <div className="flex-1 min-w-0">
                    <h2 className="font-bold text-white truncate text-sm flex items-center gap-1">
                        {activeConv?.counterpartName} {activeConv?.counterpartIsPro && <ShieldCheck size={12} className="text-white fill-white/20" />}
                    </h2>
                    <div className="flex items-center gap-1.5 opacity-90">
                        <div className="w-4 h-4 rounded-md overflow-hidden relative bg-white/20 shrink-0">{activeConv?.productImage && <Image src={activeConv.productImage} alt="" fill className="object-cover" />}</div>
            ...
            <div className="flex gap-1 relative">
                ...
                {showMenu && (
                    <div className="absolute top-12 right-0 bg-white shadow-xl rounded-xl border w-48 py-2 z-50 animate-in fade-in slide-in-from-top-2">
                        ...
                        <button onClick={confirmDeleteConversation} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 text-left"><Trash2 size={16} /> Supprimer</button>
                    </div>
                )}
            </div>
        </div>
                ...
                {activeConv?.messages.length === 0 && (
                    <div className="text-center py-10 animate-in fade-in">
                        <div className="bg-brand/5 border border-brand/10 p-6 rounded-3xl max-w-xs mx-auto">
                            ...
                            <p className="text-sm text-gray-600 font-medium">Lancez la discussion avec <strong>{activeConv.counterpartName}</strong> !</p>
                        </div>
                    </div>
                )}
                {activeConv?.messages.map((msg, i) => { 
                    ...
                    return (
                        <div key={msg.id || i} className={`flex items-end gap-2 ${isMe ? 'justify-end' : 'justify-start'} animate-in fade-in`}>
                            {!isMe && (
                                <div className="w-8 h-8 rounded-full bg-gray-200 overflow-hidden relative shrink-0 mb-1 border border-white">
                                    {msg.sender_avatar ? (<Image src={msg.sender_avatar} alt="" fill className="object-cover" />) : (<User size={14} className="m-auto text-gray-400" />)}
                                </div>
                            )}
                            <div className={`max-w-[70%] shadow-sm relative group overflow-hidden ${isMe ? 'bg-brand text-white rounded-2xl rounded-tr-sm' : 'bg-white text-gray-800 rounded-2xl rounded-tl-sm'}`}>
                                {isImg ? (
                                    <div className="cursor-pointer hover:opacity-90 transition bg-gray-100" onClick={() => openPreview(msg.content)}>
                                        <Image src={msg.content} alt="Photo" width={300} height={300} className="object-cover w-64 h-64" />
                                    </div>
                                ) : (
                                    <div className="px-4 py-2.5"><p className="whitespace-pre-wrap text-[15px]">{msg.content}</p></div>
                                )}
                                <div className={`flex items-center justify-end gap-1 pb-1 pr-2 text-[9px] ${isImg ? 'absolute bottom-0 right-0 w-full bg-linear-to-t from-black/50 to-transparent p-2 text-white' : (isMe ? 'text-white/70' : 'text-gray-400')}`}>
        ...
        {/* BARRE D'ENVOI - Z-INDEX ÉLEVÉ POUR PASSER AU DESSUS DU FOOTER S'IL RESTE */}
        <div className="bg-white p-2 pb-safe border-t border-gray-50 fixed bottom-0 left-0 w-full z-[100] shadow-2xl">
            <div className="max-w-md mx-auto flex items-end gap-2 bg-[#F2F4F7] p-1.5 rounded-3xl border border-transparent focus-within:border-brand/20 focus-within:bg-white transition-all">
                <input type="file" ref={fileInputRef} onChange={handleImageUpload} accept="image/*" className="hidden" />
                <button onClick={() => fileInputRef.current?.click()} className="p-2.5 text-gray-400 hover:text-brand flex items-center justify-center" disabled={isUploading}>
