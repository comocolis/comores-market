...
import { 
  ...
  Camera, X, CheckCircle2, Sparkles, Search
} from 'lucide-react'
...
export default function MessagesPage() {
  return (
    <Suspense fallback={<div className="flex justify-center pt-20 bg-gray-50 h-screen"><Loader2 className="animate-spin text-brand" /></div>}>
      <MessagesContent />
    </Suspense>
  )
}
...
function MessagesContent() {
  ...
  // RÉCUPÉRATION ET GROUPEMENT OPTIMISÉS
  const fetchAndGroupMessages = useCallback(async (userId: string) => {
    // OPTIMISATION SQL : Sélection ciblée des colonnes pour réduire le payload [cite: 2025-12-17]
    ...
    const handleParams = async () => {
        ...
        if (targetUserId && targetProductId) {
            ...
            if (existing) {
                ...
                try {
                    // OPTIMISATION : Requêtes parallèles ciblées [cite: 2025-12-17]
  ...
  useEffect(() => {
    const init = async () => {
      // On vérifie l'utilisateur créé précédemment [cite: 2025-12-09]
  ...
  if (view === 'list') {
    return (
        <div className="min-h-screen bg-[#F8FAFC] pb-24 font-sans text-gray-900 overflow-x-hidden">
            <div className="bg-brand pt-safe px-4 pb-8 sticky top-0 z-30 shadow-md rounded-b-[2.5rem]">
                ...
                <div className="relative group px-2">
                    ...
                    <input 
                        ...
                        className="w-full bg-white border-none rounded-2xl py-4 pl-12 pr-4 text-gray-900 font-bold placeholder:text-gray-300 shadow-sm outline-none" 
                    />
                </div>
            </div>
                            ...
                            className="bg-white p-4 rounded-[2rem] shadow-sm border border-white flex gap-4 items-center active:scale-[0.98] transition cursor-pointer hover:shadow-lg group">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-center mb-1">
                                        <h3 className={`text-[14px] truncate flex items-center gap-1.5 uppercase tracking-tight ${conv.unreadCount > 0 ? 'font-black text-gray-900' : 'font-bold text-gray-500'}`}>{conv.counterpartName} {conv.counterpartIsPro && <CheckCircle2 size={12} className="text-brand" />}</h3>
                                    ...
                                    <div className="flex justify-between items-end">
                                        <div className="flex flex-col min-w-0 pr-2">
                                            <p className="text-[9px] text-brand font-black uppercase tracking-[0.1em] truncate mb-0.5 opacity-50">{conv.productTitle}</p>
  ...
  // --- VUE CHAT ---
  return (
    <div className="fixed inset-0 bg-[#F8FAFC] font-sans text-gray-900 overflow-hidden flex flex-col h-screen">
        <div className="fixed top-0 left-0 w-full bg-brand px-4 pb-8 pt-safe shadow-md z-[150] rounded-b-[2.5rem]">
            <div className="flex items-center gap-3 pt-2">
                ...
                <Link href={`/profil/${activeConv?.counterpartId}`} className="flex flex-1 items-center gap-3 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                        <h2 className="font-black truncate text-[14px] text-white tracking-tight flex items-center gap-1.5 uppercase leading-none">{activeConv?.counterpartName}</h2>
        ...
        <div className="flex-1 overflow-y-auto px-4 pt-[115px] pb-[115px]" onClick={() => setShowMenu(false)}>
            <div className="flex flex-col justify-end min-h-full gap-4 pb-4 max-w-2xl mx-auto">
                <AnimatePresence>
        ...
        <div className="fixed bottom-0 left-0 w-full px-4 pb-safe z-[150]">
            <div className="max-w-2xl mx-auto flex items-end gap-3 bg-white p-3 rounded-[2.5rem] shadow-2xl shadow-black/5 border border-white mb-4">
                <input type="file" ref={fileInputRef} onChange={handleImageUpload} accept="image/*" className="hidden" />
                <button onClick={() => fileInputRef.current?.click()} className="p-4 text-gray-300 bg-gray-50 rounded-2xl active:scale-90 transition" disabled={isUploading}>
        ...
        {previewImage && (
            <div className="fixed inset-0 z-[500] bg-black animate-in fade-in duration-300">
                <button onClick={() => setPreviewImage(null)} className="absolute top-12 right-6 text-white p-3 bg-white/10 backdrop-blur-md rounded-2xl z-[510]"><X size={24} /></button>
            ...
            <div className="fixed inset-0 z-[500] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in" onClick={() => setShowDeleteModal(false)}>
                <div className="bg-white w-full max-w-sm rounded-[3rem] shadow-2xl p-10 text-center" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="flex flex-col gap-3">
                        <button onClick={handleDeleteConversation} className="w-full py-4 rounded-2xl font-black text-white bg-red-600 shadow-xl shadow-red-500/20 text-[10px] uppercase tracking-widest">Confirmer</button>
                        <button onClick={() => setShowDeleteModal(false)} className="w-full py-4 rounded-2xl font-black text-gray-400 bg-gray-50 text-[10px] uppercase tracking-widest">Annuler</button>
                    </div>
                </div>
            </div>
        )}
